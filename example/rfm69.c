...
static int16_t _resetPin;
static int16_t _dataPin;
static bool _init;
...
static void rfm69_chipSelect();
static void rfm69_chipUnselect();
...
void rfm69_reset()
{
  ...
  gpio_write(_resetPin, 1);
  delay_ms(1);
  gpio_write(_resetPin, 0);
 ...
 */
bool rfm69_init(uint16_t csPin, bool highPowerDevice)
{
  _csPin = csPin;
  _resetPin = -1;
  _init = false;
  ...
  gpio_enable(_csPin, GPIO_OUTPUT);
  gpio_write(_csPin, 1);
 ...
 * @return Number of received bytes; 0 if no payload is available.
 */
int rfm69_receive_internal(char* data, unsigned int dataLength)
{
...
void rfm69_continuousBit(bool bit)
{
  ...
  if ((RFM69_DATA_MODE_PACKET == _dataMode) || (-1 == _dataPin))
    return;

  // send low or high bit
  if (false == bit)
    gpio_write(_dataPin, 0);
  else
    gpio_write(_dataPin, 1);
}
...
void rfm69_setResetPin(uint16_t resetPin)
{
  _resetPin = resetPin;
  gpio_enable(_resetPin, GPIO_OUTPUT);
}

void rfm69_setDataPin(uint16_t dataPin)
{
  _dataPin = dataPin;
  gpio_enable(_dataPin, GPIO_OUTPUT);
}
